...
if (!empty($_SESSION['current_user'])) {
    ...
    if(!empty($where)){
        $totalRecords = mysqli_query($con, "SELECT * FROM `banner` where (".$where.")");
    }else{
        $totalRecords = mysqli_query($con, "SELECT * FROM `banner`");
    }
    $totalRecords = $totalRecords->num_rows;
    $totalPages = ceil($totalRecords / $item_per_page);
    if(!empty($where)){
        $products = mysqli_query($con, "SELECT * FROM `banner` where (".$where.") ORDER BY `id` DESC LIMIT " . $item_per_page . " OFFSET " . $offset);
    }else{
        $products = mysqli_query($con, "SELECT * FROM `banner` ORDER BY `id` DESC LIMIT " . $item_per_page . " OFFSET " . $offset);
    }
    mysqli_close($con);
    ?>
        ...
        <div class="listing-items">
            ...
            <ul>
                <li class="listing-item-heading">
                    <div style="width: 452px;" class="listing-prop listing-img">Ảnh</div>
                    <div style="width: 241px;" class="listing-prop listing-name">Tên <?=$config_title?></div>
                ...
                while ($row = mysqli_fetch_array($products)) {
                    ?>
                    <li>
                        <div class="listing-prop  listing-img-baner"><img src="../<?= $row['image'] ?>" alt="<?= $row['name'] ?>" title="<?= $row['name'] ?>" /></div>
