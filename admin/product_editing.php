            ...
            if (isset($_GET['action']) && ($_GET['action'] == 'add' || $_GET['action'] == 'edit')) {
                if (isset($_POST['name']) && !empty($_POST['name']) && isset($_POST['price']) && !empty($_POST['price'])) {
                    ...
                    if (!isset($error)) {
                        if ($_GET['action'] == 'edit' && !empty($_GET['id'])) { //Cập nhật lại sản phẩm
                            $result = mysqli_query($con, "UPDATE `product` SET `phanloai` = '" . $_POST['phanloai'] . "',`name` = '" . $_POST['name'] . "',`image` =  '" . $image . "',`nhasx` = '" . $_POST['nhasx'] . "', `price` = " . str_replace('.', '', $_POST['price']) . ", `content` = '" . $_POST['content'] . "', `last_updated` = " . time() . " WHERE `product`.`id` = " . $_GET['id']);
                        } else { //Thêm sản phẩm
                            $result = mysqli_query($con, "INSERT INTO `product`(`id`, `phanloai`, `name`, `image`, `nhasx`, `price`, `content`, `created_time`, `last_updated`) VALUES  (NULL, '" . $_POST['phanloai'] . "','" . $_POST['name'] . "','" . $image . "','" . $_POST['nhasx'] . "' ," . str_replace('.', '', $_POST['price']) . ", '" . $_POST['content'] . "', " . time() . ", " . time() . ");");
                        }
                ...
                <form id="editing-form" method="POST" action="<?= (!empty($product) && !isset($_GET['task'])) ? "?action=edit&id=" . $_GET['id'] : "?action=add" ?>"  enctype="multipart/form-data">
                    ...
                    <div class="wrap-field">
                        ...
                        <input type="text" name="phanloai" value="<?= (!empty($product) ? $product['phanloai'] : "") ?>" />
                        <div class="clear-both"></div>
                    </div>
                    <div class="wrap-field">
                        <label> Nhà Sản Xuất </label>
                        <input type="text" name="nhasx" value="<?= (!empty($product) ? $product['nhasx'] : "") ?>" />
                        <div class="clear-both"></div>
                    </div>
                    <div class="wrap-field">
